...
export function parseAssessment(notes, fallbackName = '') {
  ...
  return { customerCode, license, registrationDate, lastReviewDate, individuals };
}
...
export function buildRenewalTask(customer, item, permalink) {
  const when = formatDate(item.date);
  const person = item.person ? ` / ${item.person}` : '';
  let name;
  let lead;
  if (item.type === 'review') {
    name = `🔄 Periodic review due — ${customer} (due ${when})`;
    lead = `Periodic review/refresh for ${customer} is due as of ${when}.`;
  } else {
    name = `⚠️ Renew ${item.kind} — ${customer}${person} (expired ${when})`;
    lead = `${item.kind}${person ? ` for ${item.person}` : ''} at ${customer} expired on ${when}.`;
  }
  const notes = [
    lead,
    '',
...
export async function main(env = process.env, log = console) {
  ...
  for (const t of tasks) {
    ...
    for (const item of collectDueItems(parsed, today, cfg.reviewIntervalMonths)) {
      due.push({ item, customer: parsed.customerCode, permalink: t.permalink_url });
    }
  }
  ...
  let created = 0;
  let skipped = 0;
  for (const { item, customer, permalink } of due) {
    if (existingKeys.has(item.dedupKey)) {
      skipped += 1;
      continue;
    }
    const task = buildRenewalTask(customer, item, permalink);
    if (cfg.dryRun) {
